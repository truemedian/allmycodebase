...
-- This file has been patched to work with the zig build system
-- attrib.lua makes too many assumptions about the file system and cwd to work
...
do
  ...
  local f = coroutine.wrap(assert(loadfile(arg[1] or 'big.lua')))
  assert(f() == 'b')
  assert(f() == 'a')
end
dofile('nextvar.lua')
